...
function reverseWords(str) {
  const arr = str.split(' ');
  const wordArrReverse = [];
  for (let i = 0; i < arr.length; i += 1) {
    wordArrReverse.push(arr[i].split('').reverse().join(''));
  }
  return wordArrReverse.join(' ');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].match(/[A-Z]/g)) {
      invertStr += arr[i].toLowerCase();
    } else if (arr[i].match(/[a-z]/g)) {
      invertStr += arr[i].toUpperCase();
    } else {
      invertStr += arr[i];
    }
  }
...
function encodeToRot13(str) {
  const firstSample = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const secondSample = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let newStr = '';
  const arr = str.split('');
  for (let i = 0; i < arr.length; i += 1) {
    const index = firstSample.indexOf(arr[i]);
    newStr += index === -1 ? arr[i] : secondSample[index];
  }
  return newStr;
}
